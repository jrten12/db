...
- **Renovation Upgrade System**: One-time renovation per property. Cost scales with property price tier ($2.5k–$35k base), condition (Fixer-Upper 1.6x, Fair 1.2x, Good 1.0x, Excellent 0.8x), and market (terrible 0.75x to excellent 1.35x). Success rate: 40–88% base (market), +8% for poor-condition properties, −12% for excellent condition. Boost scales with condition (Fixer-Upper 1.5x), cost-to-value investment ratio, and market. Rental boosts 1–15%, flip boosts 0.5–12%. Specific renovation descriptions (kitchen, flooring, bathroom, etc.). Contractor loyalty: 3+ contractor walkthroughs → 35% chance of 5–11% discount; 5+ → 55% chance of 8–16% discount, with natural-sounding contractor quotes. Route: `POST /api/deals/:id/cosmetic-upgrade`. UI: Paintbrush "Renovate" button in TimeProgressionPanel. Stored as `cosmeticUpgradeApplied` in proFormaOutputs. Sale boost applied in both `completeFlipDeal` and `storage.sellFlipProperty`.
