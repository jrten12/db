...
function FieldRow({ label, term, rangeGuidance, rangeColor = 'gray', children, isLocked, lockMessage }: FieldRowProps) {
  return (
    <div className="grid grid-cols-[1fr,1fr,2fr] gap-4 items-center py-3 border-b border-slate-700/50 hover:bg-slate-800/30 transition-colors px-4">
      {/* Column 1: Label */}
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <input
        ...
        className="w-24 bg-slate-800 border border-slate-600 rounded-lg px-3 py-1.5 text-white text-sm font-mono text-right focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
      />
      {(prefix || suffix) && (
...
export function ProFormaEditor({ isOpen, onClose, property, inputs, onInputsChange, completedDiligence = [], onFieldTouch }: ProFormaEditorProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-2xl shadow-2xl border-2 border-blue-500/30 max-w-6xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        {/* Header */}
        <div className="bg-gradient-to-r from-blue-600 to-purple-600 px-6 py-4 flex items-center justify-between">
          <div>
            <h2 className="text-white text-2xl font-bold">{property.name}</h2>
            <p className="text-blue-100 text-sm">Purchase Price: {formatCurrency(property.price)} • {property.bedrooms}bd / {property.bathrooms}ba • {property.sizeSqft} sqft</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-4 border-b border-slate-700 bg-slate-900/50">
          <div className="flex items-center gap-4">
            <span className="text-gray-400 text-sm font-semibold">Strategy:</span>
        ...
        <div className="flex-1 overflow-y-auto">
          {/* Grid Header */}
          <div className="grid grid-cols-[1fr,1fr,2fr] gap-4 px-4 py-3 bg-slate-800/50 border-b border-slate-700 sticky top-0 z-10">
            <div className="text-gray-400 text-xs font-bold uppercase tracking-wider">Field</div>
            <div className="text-gray-400 text-xs font-bold uppercase tracking-wider text-right">Market Data / Guidance</div>
            <div className="text-gray-400 text-xs font-bold uppercase tracking-wider">Your Input</div>
          </div>

          {/* SECTION: FINANCING */}
          <div className="bg-slate-800/20">
            <div className="px-4 py-3 bg-gradient-to-r from-amber-500/10 to-transparent border-b border-amber-500/30">
          ...
          <div className="bg-slate-800/20">
            <div className="px-4 py-3 bg-gradient-to-r from-blue-500/10 to-transparent border-b border-blue-500/30">
          ...
          {isRental && (
            <div className="bg-slate-800/20">
              <div className="px-4 py-3 bg-gradient-to-r from-emerald-500/10 to-transparent border-b border-emerald-500/30">
              ...
              <div className="grid grid-cols-[1fr,1fr,2fr] gap-4 items-center py-3 border-b border-slate-700/50 hover:bg-slate-800/30 transition-colors px-4">
                ...
                <div className="text-right">
                  <span className="text-gray-400 text-xs">Adds ~$150/mo if yes</span>
                </div>
                <div className="flex items-center gap-4">
                  <button
              ...
              <div className="grid grid-cols-[1fr,1fr,2fr] gap-4 items-center py-3 border-b border-slate-700/50 hover:bg-slate-800/30 transition-colors px-4">
                ...
                <div className="text-right">
                  <span className="text-gray-400 text-xs">8-10% of rent typical</span>
                </div>
                <div className="flex items-center gap-4">
                  <button
              ...
              {inputs.propertyManagement && (
                <FieldRow
                  ...
                  term="propertyManagementPct"
                  rangeGuidance="8-10% of rent"
                  rangeColor="green"
                >
                  <SliderInput
                    value={inputs.propertyManagementPct}
                    onChange={(v) => handleChange('propertyManagementPct', v)}
                    min={5}
                    max={15}
                    step={0.5}
                    suffix="%"
        ...
        {/* Footer - Summary */}
        <div className="border-t-2 border-blue-500/30 bg-gradient-to-r from-slate-900 to-slate-800 px-6 py-4">
          <div className="grid grid-cols-2 gap-6">
            {/* Left: Key Metrics */}
            <div>
              <h4 className="text-gray-400 text-xs font-bold uppercase tracking-wider mb-3">Summary</h4>
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-gray-400 text-sm">Total Cash Needed:</span>
                  <span className="text-white font-mono font-bold text-lg">{formatCurrency(liveOutputs.totalCashInvested)}</span>
                </div>
                {isRental && (
                  <div className="flex justify-between items-center">
                    <span className="text-gray-400 text-sm">Monthly Cash Flow:</span>
                    <span className={`font-mono font-bold text-lg ${liveOutputs.cashFlowMonthly > 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                ...
                {isRental && (
                  <div className="flex justify-between items-center">
                    ...
                    <span className={`font-mono font-bold text-lg ${liveOutputs.cashOnCash >= 8 ? 'text-emerald-400' : 'text-amber-400'}`}>
