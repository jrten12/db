...
- **Tenant Leaving Mechanic**: Unfixed issues increase tenant departure probability (base 2% + 1.5% per unfixed issue + time bonus). Life situation departures also possible (1.2%/month). Departure = 1 month vacancy (rentMultiplier=0). Tenant deleted on departure, auto-created next month.
